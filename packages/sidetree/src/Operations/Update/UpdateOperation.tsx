...
import { fullyDecodedUpdateOperation } from '../../core-utils';

export interface IUpdateOperationProps extends HTMLAttributes<HTMLDivElement> {
  operation: any;
  didMethodPrefix: string;
}

export const UpdateOperation: FC<IUpdateOperationProps> = ({
  operation,
  didMethodPrefix,
}) => {
  const did = `did:${didMethodPrefix}:${operation.did_suffix}`;
  const decodedUpdateOperation = fullyDecodedUpdateOperation(operation);
            ...
            <TabPanels
              tabs={[
                ...
                },
                {
                  ...
                  panel: (
                    <JSONEditor
                      value={JSON.stringify(decodedUpdateOperation, null, 2)}
                    />
                  ),
                },
              ]}
